...
4- get a pointer to a (non copied) chunk of memory where the the queue actually is stored
...
void* acessQueueMtx(pthread_mutex_t * mtx,queueList* q,void* value,int op){
	...
	case 0:
	if(value){
	enqueueDLQueue(q,value);
	}
	break;
	case 1:
	if(q->list->currSize){
	result=dequeueDLQueue(q);
	}
	break;
	case 2:
	destroyDLQueue(q);
